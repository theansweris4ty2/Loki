...
color_change :: proc() {
	...
	change: Color_Shift
	width, height, ch: i32
	img := stb.load("lake.jpg", &width, &height, &ch, 0)
	if img == nil {
	...
	switch color {
	case "red":
		change = Color_Shift.red
	case "green":
		change = Color_Shift.green
	case "blue":
		change = Color_Shift.blue
	}


	for i: i32 = 0; i < height; i += 1 {
		for j: i32 = 0; j < width; j += 1 {
			img[(i * width + j) * ch + i32(change)] = 150
		}
	}
	// stb.flip_vertically_on_write(true)
	stb.write_png("new_waterfalls.png", width, height, ch, img, width * ch)
	stb.image_free(img)
}
